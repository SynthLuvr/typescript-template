...
- `no-unused-vars` — errors on unused variables (except `_`-prefixed args)
- `typescript/no-floating-promises` — all promises must be awaited or handled
- `typescript/return-await` — `return await` required inside try-catch

### Biome Rules

`biome.json` uses `"preset": "none"` with hand-picked rules across
four categories: `complexity`, `correctness`, `style`, and `suspicious`.
...
- `useConsistentArrowReturn` — omit `return` for single-expression arrows
...
The Vitest config (`vitest.config.ts`) includes `src/tests/**/*.test.ts`.
